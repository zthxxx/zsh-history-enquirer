...
- **中文 / emoji 等宽字符渲染对齐。** 显示宽度走 Unicode East Asian
  Width 表（`mattn/go-runewidth`），所以中日韩汉字和 emoji 都按真
  实的 2 个格子参与光标和换行计算 —— `cd 我的文档`、`echo café`、
  `git commit -m "🚀 ship"` 之类的输入都能跟 prompt 严格对齐。
