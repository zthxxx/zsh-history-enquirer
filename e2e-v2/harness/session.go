...
//     events.jsonl to e2e-v2/_artifacts/<TestName>/.
//
...
// e2e-v2/_artifacts/. The path is keyed by the testing.T's full name
...
// moduleRoot finds the e2e-v2 module root by walking upward from cwd
// looking for go.mod whose declared module path is the e2e-v2
...
func moduleRoot() string {
	...
	for i := 0; i < 8; i++ {
		...
		if data, err := os.ReadFile(modPath); err == nil {
			if strings.Contains(string(data), "/e2e-v2") {
				return dir
			}
		}
