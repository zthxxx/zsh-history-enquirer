...
Review of `e2e-v2/harness/*.go` + `e2e-v2/scenarios/basic_pick_test.go`
...
- **Where:** `e2e-v2/scenarios/basic_pick_test.go:103-113`
...
- **Where:** `e2e-v2/scenarios/basic_pick_test.go:115-141`
...
- **Where:** `e2e-v2/harness/session.go:36-40, 90`
...
- **Where:** `e2e-v2/harness/session.go:26-55` (Opts struct)
...
- **Where:** `e2e-v2/harness/session.go:449-458` (WaitExit) and
  `e2e-v2/harness/session.go:475-484` (cleanup)
...
- **Where:** `e2e-v2/harness/session.go:209-229`
- **Why:** Walks up to 8 dirs looking for `go.mod` whose contents
  match `/e2e-v2`. If the worktree is renamed or this code is
...
- **Fix:** use `runtime.Caller(0)` to locate the source file, then
  `filepath.Dir(filepath.Dir(file))` to get the e2e-v2 root.
...
- **Where:** `e2e-v2/harness/session.go:392-412`
...
- **Where:** `e2e-v2/harness/session.go:314-329`
...
e2e-v2/tapes/out/01-basic-pick.mp4 (8.1 KB)   # F: PASS
e2e-v2/tapes/out/01-basic-pick.gif (6.3 KB)   # F: PASS
```

## What the PoC got right

- **Module isolation**: separate `e2e-v2/go.mod` + no `go.work` + the
