...
- **Cancel preserves input.** Esc, Ctrl+C, and Enter-on-no-match all
  ...
  `e2e/scenarios/03_cancel_preserves_input_test.go`.
- **Tests are layered.** `internal/**/*_test.go` use property-based
  ...
  separate go module at `e2e/` (its dependencies — creack/pty,
  hinshun/vt10x — never enter the release binary's dep graph; see
  `e2e/DESIGN.md` decision 5 and the `task lint:no-test-deps`
  belt-and-braces guard). The harness drives a real `zsh -il` over
  a pty inside `e2e/docker/Dockerfile.{debian,alpine}` and
  ...
  scenarios go in `e2e/scenarios/<NN>_<topic>_test.go`.
